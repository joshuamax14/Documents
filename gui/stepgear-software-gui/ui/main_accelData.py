...
class MainPage(tk.Frame):
    ...
    async def loopDeLoop(self):
        while True:
            #get new data
            runagain=False
            ddaKneeArray,ddaFootArray = stream.readDataKnee()
            ...
            ankleFlex = distalangle - np.rad2deg(self.getAngle(ddaFootArray["proximal"],"xy"))
            ...
            if is_float(distalangle):
                if is_float(proximalangle):
                    if is_float(ankleFlex):
                        ...
                        self.footAnkleAngles[-1] = 0-(ankleFlex-self.ankleAngleOffset)
                        #print(f'state = {ddaFootArray["state"]}')
                        #print(f"ankle fex = {ankleFlex}")
                        ...
                        if (self.oldState != self.state):
                            ...
                            if self.state ==1:
                                ...
                                if self.oldState == 0:
                                    ...
                                    nElements = self.sampsize-self.heel0StrikePos
                                    filtKnee=savgol_filter(self.kneeAngles[-(nElements):],10,5)
                                    filtFoot=savgol_filter(self.footAnkleAngles[-(nElements):],10,5)
                                    #filtKnee=savgol_filter(self.kneeAngles[-(nElements+10):],8,3)[-nElements:]
                                    #filtFoot=savgol_filter(self.footAnkleAngles[-(nElements+10):],8,3)[-nElements:]
                                    self.mainPlot.clear()
                                    ...
                                    self.stepsData[self.stepsDataCount]["vertknee"]=100*((self.liftPos-self.heel0StrikePos)/len(filtKnee))
                                    ...
                                    self.stepsData[self.stepsDataCount]["vertfoot"]=100*((self.liftPos-self.heel0StrikePos)/len(filtKnee))
                                    ...
                                    self.mainPlot.plot(self.newkneerefx,self.krefy,"g-", label="normal knee flexion")
                                    self.mainPlot.plot(self.newkneerefx,self.minkneerefy,"g--")
                                    self.mainPlot.plot(self.newkneerefx,self.maxkneerefy,"g--")
                                    self.main2Plot.plot(self.newfootrefx,self.frefy,"g-", label="normal ankle flexion")
                                    for tempp in range(0, self.maxStepData):
                                        if len(self.stepsData[tempp])>0:
                                            ...
                                            self.main2Plot.plot(xnew,self.stepsData[tempp]["datafoot"],self.lineorders[tempp])
                                    ...
                                    #self.main2Plot.set_ylim(-50, 50)
                                    self.mainPlot.legend(loc='best')
                                    self.main2Plot.legend(loc='best')
                                    self.bigfigcanvas.draw()
                                    self.bigfig.savefig(f"./charts/{self.controller.gensession}_step{self.stepCounter}.png")
                                    self.stepCounter +=1
                                    self.imgCount+=1
                                #else:
                                #    exit()
    ...
    def __init__(self, parent, controller):
        ...
        self.main2Plot = self.bigfig.add_subplot(212)
        self.mainPlot.set_title("Knee Flex - Ext [deg]")
        self.main2Plot.set_title("Ankle Flex - Ext [deg]")
        # plotting the graph
        self.kneeline, = self.mainPlot.plot(np.zeros(self.sampsize),"r+", label="Knee Flex Angle")
        self.ankleline, = self.main2Plot.plot(np.zeros(self.sampsize),"r+", label = "Ankle Flex Angle")
