            ...
            if self.controller.sysReady:
                ...
                if len(footProx)==4:
                    ...
                    if len(kneeProx)==4:
                        ...
                        #cleandat["y_interpolatedknp"] = pd.Series(cleandat["nooutliersknp"]).interpolate()
                        #cleandat["y_interpolatedknd"] = pd.Series(cleandat["nooutliersknd"]).interpolate()
                        #cleandat["y_interpolatedfp"] = pd.Series(cleandat["nooutliersfp"]).interpolate()
                        #print("orig",self.kneeProx[-(SPAN*10):])
                        #print("clipper",cleandat["clippedknp"])
                        #print("no outliers",cleandat["nooutliersknp"])
                        #print("y_interpolatedknp",cleandat["y_interpolatedknp"])
                        self.cleankneeProx[:-(SPAN*5)]=self.cleankneeProx[(SPAN*5):]
                        #self.cleankneeProx[-(SPAN*5):]=self.kneeProx[-(SPAN*5):] #np.asarray(cleandat["y_interpolatedknp"])
                        self.cleankneeProx[-(SPAN*5):]=np.asarray(cleandat["y_interpolatedknp"])
                        self.cleankneeDist[:-(SPAN*5)]=self.cleankneeDist[(SPAN*5):]
                        #self.cleankneeDist[-(SPAN*5):]=self.kneeDist[-(SPAN*5):] #np.asarray(cleandat["y_interpolatedknd"])
                        self.cleankneeDist[-(SPAN*5):]=np.asarray(cleandat["y_interpolatedknd"])
                        self.cleanfootProx[:-(SPAN*5)]=self.cleanfootProx[(SPAN*5):]
                        #self.cleanfootProx[-(SPAN*5):]=self.footProx[-(SPAN*5):] #np.asarray(cleandat["y_interpolatedfp"])
                        ...
                        for n in range(0,4):
                            ...
                            #diff = (self.cleankneeProx[(self.sampsize-4)+n]-self.cleankneeDist[(self.sampsize-4)+n])
                            ...
                            diff = (14+self.cleanfootProx[(self.sampsize-4)+n])+(self.cleankneeDist[(self.sampsize-4)+n]+90)
                            #diff = self.cleankneeDist[(self.sampsize-4)+n]-self.cleanfootProx[(self.sampsize-4)+n]
                            #diff = self.footProx[n] - self.kneeDist[n]
                            #if footProx[n]
                        ...
                        if self.delay==0:
                            ...
                            #for z in range(1,self.sampsize):
                                ...
                                if self.fStates[n]==1:
                                    if (self.fStates[n-1] == 0) and (self.fStates[n-2] == 0):
                                    #found valid heelstrike
                                        if endfound==False:
                                            #found the end
                                            endfound=True
                                            endindex=n #+dell #+((SYNC_DELAY-0)*2)
                                        else:
                                            #found start of heelstrike
                                            if endindex-n>10:
                                                startindex = n #+dell  #+((SYNC_DELAY-0)*2)
                                                startfound=True
                                                break
                                #elif self.fStates[n]==0:
                                #    if self.fStates[n-1]==2:
                                #        #if endfound:
                                #        if toesofffound==False:
                                #            toesofffound=True
                                #            if self.fStates[n+1]==0:
                                #                toeoffindex=n -1 #+dell #-((SYNC_DELAY-0)*4)
                            rawkneeFlex=[]
                            rawankleFlex=[]
                            rawtoeoff=[]
                            #toeoffLocation= 
                            if (startfound):
                                ...
                                startindex-=int(dell/4)
                                endindex+=dell
                                #endindex=len(self.fStates)-1
                                toeoffindex+=dell
                                ...
                                filtKnee=savgol_filter(self.kneeFlexion,16,3)
                                #filtKnee=savgol_filter(self.kneeFlexion,6,3)
                                filtFoot=savgol_filter(self.ankleFlexion,16,3)
                                #filtFoot=savgol_filter(self.ankleFlexion,12,3)
                                ...
                                for tempp in range(0, self.maxStepData):
                                    if len(self.stepsData[tempp])>0:
                                        ...
                                        self.mainPlot.axvline(x=self.stepsData[tempp]["vertfoot"], color='blue', linestyle='-')
                                        self.main2Plot.axvline(x=self.stepsData[tempp]["vertfoot"], color='blue', linestyle='-')
                                ...
                                if 100*((toeoffindex-startindex)/(nElements))>0:
                        ...
                        #proceed plotting
                        mmin = np.min([np.min(self.fStates),np.min(self.kneeFlexion),np.min(self.ankleFlexion)])
                        mmax = np.max([np.max(self.fStates),np.max(self.kneeFlexion),np.max(self.ankleFlexion)])
                        ...
                        self.kneeFlexionlineRT.set_ydata((self.kneeFlexion[-200:]))
